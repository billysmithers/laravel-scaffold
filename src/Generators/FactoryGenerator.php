...
class FactoryGenerator extends BaseGenerator
{
    ...
    protected function generateFields(): string
    {
        ...
        foreach ($this->config->fields as $field) {
            if ($field->isPrimary && $field->dbType === 'integer') {
                continue;
            }

            if ($field->isPrimary && in_array($field->dbType, ['uuid', 'string'])) {
                $fields[] = "'".$field->name."' => ".'$this->faker->uuid';
                continue;
            }
            ...
            switch (strtolower($field->dbType)) {
                ...
                case 'json':
                    $fieldData = "'".$field->name."' => "
                        ."json_encode([['type' => 'paragraph', 'children' => [['text' => 'I am some rich text']]]])";
                    $fakerData = '';
                    break;
                default:
                    $fakerData = 'word';
